...
describe("Request validation middleware", () => {
  ...
  const defaultRequest = {
    ...
    host: fullUrl.host,
    headers: {
      "X-Reach-Signature": defaultSignature,
      host: fullUrl.host,
    },
  ...
  const defaultRequestWithoutReachSignature = {
    method: "POST",
    protocol: fullUrl.protocol,
    host: fullUrl.host,
    headers: {
  ...
  it("should validate post body if given a query param", (done) => {
    const request = httpMocks.createRequest(
      Object.assign({}, defaultRequest, {
        ...
        body,
        headers: Object.assign({}, defaultRequest.headers, {
          "X-Reach-Signature": requestUrlWithHashSignature,
        }),
      })
    );
  ...
  it("should fail if no Reach signature is provided in the request headers", () => {
    const newUrl =
      fullUrl.pathname + fullUrl.search + "&somethingUnexpected=true";
    const request = httpMocks.createRequest(
      Object.assign({}, defaultRequestWithoutReachSignature, {
        originalUrl: newUrl,
      })
    );
