...
export interface RequestValidatorOptions {
  /**
   * The full URL (with query string) you used to configure the webhook with Reach - overrides host/protocol options
   */
  url?: string;
  /**
   * Manually specify the host name used by Reach in a number's webhook config
   */
  host?: string;
  /**
   * Manually specify the protocol used by Reach in a number's webhook config
   */
  protocol?: string;
}
...
export interface WebhookOptions {
   ...
   * came from Reach.  Default true. If the request does not originate from
   * Reach, we will return a text body and a 403.  If there is no configured
 ...
 *
 * @param parsedUrl - The parsed url object that Reach requested on your server
...
/**
 Utility function to validate an incoming request is indeed from Reach

 @param authToken - The auth token, as seen in the Reach portal
 @param reachHeader - The value of the X-Reach-Signature header from the request
...
export function validateRequest(
   ...
   */
  const signatureWithPort = getExpectedReachSignature(
    authToken,
    urlWithPort,
    params
  );
  const signatureWithoutPort = getExpectedReachSignature(
    authToken,
    urlWithoutPort,
    params
  );
  const validSignatureWithPort = scmp(
    Buffer.from(reachHeader),
...
/**
 Utility function to validate an incoming request is indeed from Reach. This also validates
...
export function validateRequestWithBody(
  ...
  return (
    validateRequest(authToken, reachHeader, url, {}) &&
...
/**
 ...
 @param opts - options for request validation:
    -> url: The full URL (with query string) you used to configure the webhook with Reach - overrides host/protocol options
    -> host: manually specify the host name used by Reach in a number's webhook config
    -> protocol: manually specify the protocol used by Reach in a number's webhook config
 */
export function validateIncomingRequest(
  ...
  if (webhookUrl.indexOf("bodySHA256") > 0) {
    return validateRequestWithBody(
      authToken,
      request.header("X-Reach-Signature") || "",
      webhookUrl,
      request.rawBody || "{}"
    );
  } else {
...
/**
Express middleware to accompany a Reach webhook. Provides Reach
...
Examples:
var webhookMiddleware = reach.webhook();
var webhookMiddleware = reach.webhook('asdha9dhjasd'); //init with auth token
var webhookMiddleware = reach.webhook({
    validate:false // don't attempt request validation
});
var webhookMiddleware = reach.webhook({
    host: 'hook.reach.talkylabs.com',
    protocol: 'https'
});
 */
export function webhook(
  ...
  if (options) {
    options.authToken = tokenString
      ? tokenString
      : process.env.REACH_TALKYLABS_API_KEY;
  }
  // Create middleware function
  return function hook(request, response, next) {
    // Do validation if requested
    if (options?.validate) {
      // Check if the 'X-Reach-Signature' header exists or not
      if (!request.header("X-Reach-Signature")) {
        return response
          .type("text/plain")
          .status(400)
          .send(
            "No signature header error - X-Reach-Signature header does not exist, maybe this request is not coming from Reach."
          );
      }
      // Check for a valid auth token
      if (!options?.authToken) {
        console.error(
          "[Reach]: Error - Reach auth token is required for webhook request validation."
        );
        response
          .type("text/plain")
          .status(500)
        ...
        // Check that the request originated from Reach
        ...
        if (valid) {
          ...
          return response
            .type("text/plain")
            .status(403)
            .send("Reach Request Validation Failed.");
        }
      }
    } else {
      next();
