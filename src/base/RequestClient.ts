...
class RequestClient {
  ...
  constructor(opts?: RequestClient.RequestClientOptions) {
    ...
    if (process.env.REACH_TALKYLABS_CA_BUNDLE !== undefined) {
      if (agentOpts.ca === undefined) {
        agentOpts.ca = fs.readFileSync(process.env.REACH_TALKYLABS_CA_BUNDLE);
      }
    }
  ...
  request<TData>(
    ...
    if (opts.username && opts.password) {
      ...
      headers.ApiUser = opts.username;
      headers.ApiKey = opts.password;
    }
  ...
  filterLoggingHeaders(headers: Headers) {
    return Object.keys(headers).filter((header) => {
      ...
      return !lowerHeader.includes("authorization") && !lowerHeader.includes("apiuser") && !lowerHeader.includes("apikey");
    });
  }

  private logRequest<TData>(options: LastRequestOptions<TData>) {
    console.log("-- BEGIN Reach API Request --");
    ...
    console.log("-- END Reach API Request --");
  }
}
