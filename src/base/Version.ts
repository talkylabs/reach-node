...
import Page, { ReachResponsePayload } from "./Page";
import { RequestOpts } from "./BaseReach";
...
export default class Version {
  ...
  /**
   * Get Url without pagination information
   *
   * @param url - url to transform
   * @param url - query parameters
   * @returns transformed url
   */
  urlWithoutPaginationInfo(url: string, params?: object): string {
    let aUrl = new URL(url);
    let query = params!=null ? qs.stringify(params, { arrayFormat: "repeat" }): "";
    if(!((aUrl.search == null) || (aUrl.search.length==0))){
      query = aUrl.search + (aUrl.search.length>1 && query.length>0?"&":"") + query;
      query = query.substring(1); // remove the initial "?"
    }
    if(query.length==0){
      return url;
    }
   ...
   * @returns promise that resolves to created record
   */
  send(opts: RequestOpts): Promise<any> {
    var qResponse = this.request(opts);
    qResponse = qResponse.then(function success(response) {
      if (response.statusCode < 200 || response.statusCode >= 300) {
        throw new RestException(response);
   ...
   * @returns promise that resolves to created record
   */
  start(opts: RequestOpts): Promise<any> {
    var qResponse = this.request(opts);
    qResponse = qResponse.then(function success(response) {
      if (response.statusCode < 200 || response.statusCode >= 300) {
        throw new RestException(response);
   ...
   * @returns promise that resolves to created record
   */
  check(opts: RequestOpts): Promise<any> {
    var qResponse = this.request(opts);
    qResponse = qResponse.then(function success(response) {
      if (response.statusCode < 200 || response.statusCode >= 300) {
        throw new RestException(response);
   ...
   * @returns promise that resolves to updated result
   */
  unschedule(opts: RequestOpts): Promise<any> {
    var qResponse = this.request(opts);
    qResponse = qResponse.then(function success(response) {
      if (response.statusCode < 200 || response.statusCode >= 300) {
        throw new RestException(response);
