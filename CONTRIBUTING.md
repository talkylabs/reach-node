# Contributing to `reach-node`

We'd love for you to contribute to our source code and to make `reach-node`
...
Help us keep `reach-node` open and inclusive. Please be kind to and considerate
of other developers, as we all have the same goal: make `reach-node` as good as
it can be.

## <a name="question"></a> Got an API/Product Question or Problem?

If you have questions about how to use `reach-node`, please see our
...
* **Major Changes** that you wish to contribute to the project should be
  discussed first with `reach-node` contributors in an issue or pull request so
...
* **`reach-node` Version(s)** - is it a regression?
...
* Run the full `reach-node` test suite (aliased by `make test`), and ensure
  that all tests pass.
...
In GitHub, send a pull request to `reach-node:main`.
If we suggest changes, then:

* Make the required updates.
* Re-run the `reach-node` test suite to ensure tests are still passing.
...
[issue-link]: https://github.com/talkylabs/reach-node/issues/new
[github]: https://github.com/talkylabs/reach-node
